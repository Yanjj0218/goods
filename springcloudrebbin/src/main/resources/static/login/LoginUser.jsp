...
<script>	
	...
	function login(){
		$.ajax({
			...
			type:'post',
			data:{
				loginNumber:$("#login-username").val(),
	...
	function next(){
		$('#imgsrc').attr('src','<%=request.getContextPath()%>/verificationCode?t='+Date.parse(new Date()));
	}
	
	function goto_Quick_login(){
		location.href="../login/QuickLogin.jsp"
	}
				...
				<form class="container offset1 loginform">
					...
					<div class="pad input-container">
						<section class="content">
							...
							<td> 
								<img id="imgsrc" src="<%=request.getContextPath()%>/verificationCode">
								<input type="text" size="5" id="imgcode"/>
								<a href="javascript:next();">看不清，换一张</a>
							</td>
						</section>
					</div>
					<div class="form-actions">
						<a tabindex="4" class="btn pull-left btn-link text-muted" onClick="goto_forget()">忘记密码?</a>
						<a tabindex="5" class="btn pull-left btn-link text-muted" onClick="goto_Quick_login()">快捷登录!</a>
